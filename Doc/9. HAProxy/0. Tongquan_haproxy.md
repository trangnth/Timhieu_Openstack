# Tông quản về HAProxy và Load Balancing 

## 1. Tổng quan

HAProxy (High Availability Proxy) là một phần mềm mã nguồn mở phổ biến TCP/HTTP Load Balancer và giải pháp proxy có thể chạy trên nhiểu nền tảng như Linux, FreeBSD và Solaris. HAProxy giúp cải thiện hiệu suất và độ tin cậy của môi trường máy chủ bằng cách phân phối khối lượng công việc đều trên các server ví dụ (web, application, database)
...
* **roundrobin**: lựa chọn lần lượt các server. Mặc định sẽ sử dụng thuật toán này.
* **leastconn**: Lựa chọn server ít số lượng kết nối hơn, nó được khuyến khích cho các session dài hơn. các server trong cùng một backend cũng được rotated theo kiểu round-robin.
* **source**: Lựa chọn server dựa trên sự phân tích source IP (địa chỉ IP của người sử dụng). Đây là method đảm bảo người dùng sẽ được kết nối tới dùng một server.
