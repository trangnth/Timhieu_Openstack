...
HAProxy (High Availability Proxy) là một phần mềm mã nguồn mở phổ biến TCP/HTTP Load Balancẻ và giải pháp proxy có thể chạy trên nhiểu nền tảng như Linux, FreeBSD và Solaris. HAProxy giúp cải thiện hiệu suất và độ tin cậy của môi trường máy chủ bằng cách phân phối khối lượng công việc đều trên các server ví dụ (web, application, database)
...
### 3.5 Những thứ có thể làm với ACLs
